...
def plot_learning_curve2(x, scores, figure_file):
    running_avg = np.zeros(len(scores))
    for i in range(len(running_avg)):
        running_avg[i] = np.mean(scores[max(0, i-100):(i+1)])
    plt.style.use('ggplot')  # This will change the style including colors to a pre-defined set known as 'ggplot'
    plt.plot(x, running_avg)  # Color is automatically adjusted by the style
    plt.title('Running Average of Previous 100 Scores')
    plt.xlabel('Episodes')
    plt.ylabel('Average Score')
    plt.savefig(figure_file)   

#gravity=random.uniform(-12, 0)
#wind_power=random.uniform(0, 20)
...
if __name__ == '__main__':
    ...
    # After all games are played, plot the results
    episodes = list(range(n_games))
